...
object GraphTest {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val usersRDD: RDD[(String, Map[String, String])] = sc.parallelize(
      List(
        ("M001", Map(("imei", "IM001"), ("mac", "MA001"), ("openudid",
          "OU001"), ("idfa", "ID001"))),
        ("M002", Map(("mac", "MA002"), ("openudid", "OU001"))),
        ("M003", Map(("imei", "IM003"), ("androidid", "AN003"), ("idfa",
          "ID001"))),
        ("M004", Map(("imei", "IM004"), ("mac", "MA004"), ("openudid",
          "OU004"))),
    ...
    // TODO: 2、图中顶点数据, 顶点属性为
    val idVerticesRDD: RDD[(VertexId, String)] = usersRDD.flatMap { case (mainId, idsMap) =>

      var list = new ListBuffer[(VertexId, String)]()
      // 主标识ID
      list += mainId.hashCode.toLong -> s"mainid->$mainId"
      // 其他标识ID
      idsMap.foreach { case (idKey, idValue) =>
    ...
    // TODO: 5、从图中获取连通图
    // http://spark.apache.org/docs/2.2.0/graphx-programming-guide.html#connected-components
